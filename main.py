...
COR_DE_FUNDO = (187, 173, 160)    # U1 - Variáveis e atribuições / U2 - Tuplas: cor armazenada como tupla RGB
COR_DO_TEXTO = (119, 110, 101)    # U1 - Variáveis e atribuições / U2 - Tuplas: cor armazenada como tupla RGB
TEXT0_CLARO = (249, 246, 242)     # U1 - Variáveis e atribuições / U2 - Tuplas: cor armazenada como tupla RGB
...
##############################################################################################################
FONTE_GRANDE = pygame.font.Font(None, 55)                   # U3 - Estudo de casos (PyGame): criando objeto de fonte
FONTE_MEDIA = pygame.font.Font(None, 40)                    # U3 - Estudo de casos (PyGame): criando objeto de fonte
FONTE_PEQUENA = pygame.font.Font(None, 30)                  # U3 - Estudo de casos (PyGame): criando objeto de fonte

############################################################################
# Criando a janela do jogo e o relógio para controlar a taxa de atualização
############################################################################
janela = pygame.display.set_mode((LARGURA_JANELA, ALTURA_JANELA))           # U3 - Estudo de casos (PyGame): criando janela gráfica
clock = pygame.time.Clock()                                                 # U3 - Estudo de casos (PyGame): criando objeto de controle de FPS
...
def comprimir(linha):                                       # U3 - Funções: definição com parâmetro e retorno
    ...
    # preenchendo o restante com zeros
    ###################################
    nova_linha += [0] * (TAMANHO_GRADE - len(nova_linha))   # U1 - Expressões: concatenação e multiplicação de lista; U1 - Tipos de dados: listas
...
def desenhar_jogo(estado_de_jogo):                          # U3 - Funções: definição com parâmetro
    tabuleiro = estado_de_jogo["tabuleiro"]                 # U2 - Dicionários: acesso a valor por chave
    janela.fill((250, 248, 246))                            # U3 - Estudo de casos (PyGame): preenchendo fundo da janela com cor RGB

    ##################
    # Título do jogo
    ##################
    titulo = FONTE_GRANDE.render("2048", True, COR_DO_TEXTO)                # U3 - Estudo de casos (PyGame): renderizando texto; U1 - Strings
    janela.blit(titulo, (20, 20))                                           # U3 - Estudo de casos (PyGame): desenhando superfície na janela
    ...
    for i in range(TAMANHO_GRADE):                                                  # U2 - Estruturas de controle: for: iterando sobre linhas
        for j in range(TAMANHO_GRADE):                                              # U2 - Estruturas de controle: for aninhado: iterando sobre colunas
            ...
            valor = tabuleiro[i][j]                                                 # U1 - Variáveis e atribuições: lendo valor da célula na lista 2D
            ...
            #######################################################
            pygame.draw.rect(janela, color, (x, y, TAMANHO_CELULA, TAMANHO_CELULA)) # U3 - Estudo de casos (PyGame): desenhando retângulo
            ...
            if valor != 0:                                                          # U1 - Estruturas de controle: if: verificando célula não vazia
                if valor <= 4:                                                      # U1 - Estruturas de controle: if-elif-else: escolhendo tamanho da fonte
                    texto = FONTE_GRANDE.render(str(valor), True, COR_DO_TEXTO)     # U3 - Estudo de casos (PyGame): renderizando número como texto
                elif valor < 1024:                                                  # U1 - Estruturas de controle: elif
                    texto = FONTE_MEDIA.render(str(valor), True, COR_DO_TEXTO)      # U3 - Estudo de casos (PyGame): renderizando com fonte média
                else:                                                               # U1 - Estruturas de controle: else
                    texto = FONTE_PEQUENA.render(str(valor), True, TEXT0_CLARO)     # U3 - Estudo de casos (PyGame): renderizando com fonte pequena

                texto_rect = texto.get_rect(
                    center=(x + TAMANHO_CELULA // 2, y + TAMANHO_CELULA // 2)       # U3 - Estudo de casos (PyGame): centralizando texto na célula; U1 - Expressões
                )
                janela.blit(texto, texto_rect)                                      # U3 - Estudo de casos (PyGame): desenhando texto na tela
    ...
    legenda = [                                                             # U1 - Tipos de dados: listas: lista de superfícies de texto
        FONTE_PEQUENA.render(
            "Use WASD ou as setas para movimentar", True, (150, 150, 150)   # U3 - Estudo de casos (PyGame): renderizando instrução; U1 - Strings
        ),
        FONTE_PEQUENA.render("R para reiniciar", True, (150, 150, 150)),    # U3 - Estudo de casos (PyGame): renderizando instrução; U1 - Strings
    ]

    ############################################
    # Funções que escrevem as intruções na tela
    #############################################
    janela.blit(legenda[0], (tabuleiro_left, ALTURA_JANELA - 55))           # U3 - Estudo de casos (PyGame): desenhando legenda; U1 - Listas: acesso por índice
    janela.blit(legenda[1], (tabuleiro_left, ALTURA_JANELA - 30))           # U3 - Estudo de casos (PyGame): desenhando legenda; U1 - Listas: acesso por índice
...
def main():                                                                 # U3 - Funções: definição da função principal sem parâmetros
    ...
    pygame.display.set_caption("Pygame 2048")                               # U3 - Estudo de casos (PyGame): definindo título da janela; U1 - Strings
    ...
    while running:                                                          # U2 - Estruturas de controle: while: loop principal do jogo
        ...
        clock.tick(60)                                                      # U3 - Estudo de casos (PyGame): limitando FPS; U1 - Tipos de dados: números
